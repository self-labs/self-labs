  ...
  for (const conta of contas) {
    try {
      const repos = await api(
        `${conta.caminho}?per_page=100&type=owner&sort=pushed`,
      );
...
async function main() {
  ...
  console.log(
    token
      ? "Consultando a API do GitHub (autenticado)"
      : "Consultando a API do GitHub (anonimo)",
  );

  try {
    const repos = await listarRepos();
    if (repos.length === 0)
      throw new Error("nenhum repositorio publico retornado");
    ...
    for (const repo of repos) {
      ...
      commits += n;
      console.log(
        `  ${repo.full_name.padEnd(34)} ${String(n).padStart(4)} commits`,
      );
    }
